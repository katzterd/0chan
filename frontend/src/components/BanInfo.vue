<template>
    <div v-if="ban" class="block ban-info">
        ...
        <div v-else>
            <span v-if="ban.appeal"
                >Объяснительная: <i>{{ ban.appeal }}</i></span
            >
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        sendAppeal() {
            ...
            Moderation.appealBan(this.ban.id, this.appeal).then((response) => {
                if (response.data.ok) {
                    this.$bus.emit(
                        BusEvents.ALERT_INFO,
                        "Мочераторы подумают, но ничего не обещают."
                    );
                }
            });
            this.close();
