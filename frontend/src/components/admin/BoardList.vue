<template>
    <div>
        <Headline>
            ...
            <span slot="buttons">
                <router-link
                    :to="{ name: 'admin_newBoard' }"
                    class="btn btn-primary"
                >
                    <i class="fa fa-plus"></i>
                    <span class="hidden-xs">Создать доску</span>
                </router-link>
            </span>
        </Headline>
...
export default {
    ...
    methods: {
        fetch() {
            this.boards = null;
            return Management.list().then((response) => {
        ...
        deleteBoard(board) {
            if (
                !confirm(
                    `Удалить доску /${board.dir}/ "${board.name}"? Восстановить ее будет невозможно.`
                )
            ) {
                return;
            }
