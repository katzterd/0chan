<template>
    <div class="site-manage">
        <Headline>
            <span slot="title"> Глобальные настройки сайта </span>
        </Headline>
...
export default {
    ...
    methods: {
        ...
        saveSettings() {
            Globals.save(this.settings_data).then((response) => {
                if (response.data.ok) {
                    this.$bus.emit(
                    ...
                    if (response.data.errors) {
                        response.data.errors.forEach((element) =>
                            this.$bus.emit(BusEvents.ALERT_ERROR, element)
                        );
                    }
                }
            });
        ...
        saveSpamlist() {
            Globals.spamlistSave(this.spamlist_data).then((response) => {
                if (response.data.ok) {
                    this.$bus.emit(
                        BusEvents.ALERT_SUCCESS,
                        "Спамлист сохранен"
