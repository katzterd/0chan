...
export default {
    ...
    methods: {
        fetch() {
            return Management.board(this.dir).then((response) => {
        ...
        onSubmit() {
            ...
            Management.board(this.dir, this.board).then((response) => {
        ...
        changeOwner() {
            ...
            if (
                !confirm(
                    "Вы потеряете доступ к управлению этой доской. Продолжить?"
                )
            ) {
                return;
            }
            this.submitting = true;
            Management.changeOwner(this.dir, this.newOwner).then((response) => {
                if (response.data.ok) {
                    this.$bus.emit(
                        BusEvents.ALERT_SUCCESS,
                        `Доступ к /${this.dir}/ передан ${this.newOwner}`
                    );
                    this.$router.push({ name: "admin" });
                } else {
                    this.$bus.emit(
                        BusEvents.ALERT_ERROR,
                        response.data.reason || "Ошибка"
                    );
                }
                this.submitting = false;
            });
