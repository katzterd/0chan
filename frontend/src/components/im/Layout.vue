<template>
    <div>
        <Headline>
            ...
            <span slot="buttons">
                <button
                    class="btn btn-primary hidden-lg"
                    @click="isDialogsView = !isDialogsView"
                >
                    <i class="fa fa-fw fa-list"></i>
                    <span class="btn-caption hidden-xs"
                        >{{
                            isDialogsView ? "Скрыть" : "Показать"
                        }}
                        диалоги</span
                    >
                </button>
                <button
                    class="btn btn-default hidden visible-lg"
                    @click="isSwapLayout = !isSwapLayout"
                >
        ...
        <div class="im-box">
            <div
                class="dialog-list"
                :class="{
                    'hidden-xs hidden-sm hidden-md': !isDialogsView,
                    'swap-layout': isSwapLayout,
                }"
            >
    ...
    methods: {
        fetch() {
            ...
            promise = promise.then(
                this.$nextTick(() => this.$refs.list.fetch())
            );
