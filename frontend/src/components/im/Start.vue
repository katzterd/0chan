<template>
    <div class="panel panel-default">
        <div class="panel-heading">
            ...
            <b style="padding-left: 20px"> Начало диалога </b>
        </div>
        <div class="panel-body">
            <div v-if="notFound" class="empty-page">
                <h4>
                    Адрес
                    <span class="im-address im-grey">{{ toAddress }}</span> не
                    найден
                </h4>
                <br />
                ...
                <form @submit.prevent="gotoDialog" class="form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="vspace list-group">
                            <router-link
                                ...
                                :to="{
                                    ...
                                    params: {
                                        ...
                                        asAddress: startAs.address,
                                    },
                                }"
                            >
                                <div class="row">
                                    <div
                                        class="col-md-12 list-group-item-heading"
                                    >
                                        <span class="im-address">{{
                                            startAs.address
                                        }}</span>
                                        {{ startAs.name }}
                                    </div>
                                    <div
                                        class="col-md-12 list-group-item-text text-right"
                                    >
                                        <span
                                            v-if="startAs.last"
                                            class="color-grey-dk"
                                            :title="startAs.last | timestamp"
                                        >
                                            посл. сообщение:
                                            {{ startAs.last | timeago }}
                                        </span>
                            ...
                            <a
                                href="#"
                                @click.prevent="showAddIdentity"
                                class="list-group-item"
                            >
                                <i class="fa fa-plus"></i> Создать новую
                                личность
                            </a>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        fetch() {
            this.notFound = false;
            this.ready = false;
            return Dialog.start(this.toAddress).then((response) => {
                if (response.data.ok) {
        ...
        gotoDialog() {
            this.$router.push({
                name: "im",
                params: { to: this.toAddress, as: this.asAddress },
            });
        },
    },
};
