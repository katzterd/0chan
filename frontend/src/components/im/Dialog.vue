                ...
                <div class="text-center vspace" v-if="hasBefore">
                    ...
                    >
                        <button
                            ...
                            @click.prevent="fetchMessagesBefore"
                            :disabled="fetchingBefore"
                        >
                            <span v-if="fetchingBefore">
                                <i class="fa fa-spinner fa-spin fa-fw"></i>
                                подгружаем более ранние сообщения
                            </span>
                            <span v-if="!fetchingBefore">
                                <i class="fa fa-ellipsis-h"></i> подгрузить
...
export default {
    ...
    methods: {
        ...
        fetchMessagesBefore() {
            ...
            return Dialog.get(
                ...
                if (response.data.ok) {
                    ...
                    this.messages = response.data.messages.concat(
                        this.messages
                    );
                    this.hasBefore = response.data.hasBefore;
                    this.$nextTick(() => {
                        const newScrollHeight =
                            this.$refs.scrollable.scrollHeight;
        ...
        scrollToBottom(slow) {
            $(this.$refs.scrollable).animate(
                { scrollTop: this.$refs.scrollable.scrollHeight },
                slow ? 200 : 0
            );
        },
        deleteDialog() {
            ...
            this.sending = true;
            return Dialog.deleteDialog(this.dialogId).then((response) => {
