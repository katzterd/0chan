<template>
    <div v-if="identities" class="panel panel-default">
        <div class="panel-body">
                    ...
                    <tbody>
                        <tr v-for="id in identities">
                            <td width="100%">
                                <span
                                    class="im-address"
                                    v-clipboard="id.address"
                                    @success="addressCopied"
                                    >{{ id.address }}</span
                                >
                            ...
                            <td>
                                <div class="pull-right" style="display: flex">
                                    <a
                                        class="im-button btn-sm fa fa-search"
                                        @click.prevent="
                                            $emit('search', id.address)
                                        "
                                    ></a>
                                    <a
                                        class="im-button btn-sm fa fa-clipboard"
                                        v-clipboard="id.address"
                                        @success="addressCopied"
                                    ></a>
                                    <a
                                        class="im-button btn-sm fa fa-link"
                                        v-clipboard="id.link"
                                        @success="linkCopied"
                                    ></a>
                                    <a
                                        class="im-button btn-sm fa fa-trash"
                                        @click.prevent="
                                            deleteIdentity(id.address)
                                        "
                                    ></a>
                                </div>
                            </td>
            ...
            <div
                class="row text-muted center-block"
                style="max-width: 600px; padding: 20px 40px 50px"
            >
                Подсказка:
                <ul class="vspace">
                    <li>
                        Чтобы оставить адрес в посте, нужно просто вставить его
                        как есть.
                    </li>
                    <li>
                        Чтобы оставить адрес на другом сайте или передать иным
                        способом, можно использовать ссылку вида:
                        {{ exampleLink }}
                    </li>
                </ul>
            </div>
...
export default {
    ...
    methods: {
        ...
        deleteIdentity(address) {
            if (
                !confirm(
                    "Личность удаляется окончательно с отвязкой адреса от аккаунта. Восстановить нельзя. Удаляем?"
                )
            )
                return;
            return Dialog.deleteIdentity(address).then((response) => {
