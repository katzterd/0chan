        ...
        <div class="dialog-list-filter">
            <div class="form-group has-feedback">
                <input
                    ...
                    v-model="listFilter"
                    type="text"
                    placeholder="Поиск по адресатам"
                />
                <span
                    :class="
                        listFilter
                            ? 'search-filled glyphicon-remove'
                            : 'glyphicon-search'
                    "
                    @click="listFilter = ''"
            ...
            <div class="list-group" v-if="filteredDialogs.length > 0">
                <router-link
                    ...
                    :to="{
                        name: 'im_dialog',
                        params: {
                            toAddress: dialog.with.address,
                            asAddress: dialog.as.address,
                        },
                    }"
                >
                    <div
                        v-if="dialog.unreadCount > 0"
                        class="dialog-list-count-unread"
                    >
                        +{{ dialog.unreadCount }}
                    </div>
                    ...
                    <div style="font-size: 0.8em">
                        <div class="color-grey-lt">
                            <span
                                v-if="
                                    dialog.lastMessage.from.address ==
                                    dialog.as.address
                                "
                                >отправлено</span
                            >
...
export default {
    ...
    methods: {
        ...
        fetchDialogs() {
            return Dialog.list().then((response) => {
