<template>
    <div class="sidebar" :class="{ 'mobile-open': visible, hidden }">
        <button
            class="btn btn-link visible-xs-inline-block btn-hide-sidebar"
            @click="visible = false"
        >
        ...
        <div class="sidemenu-boards">
            <form class="form-horizontal" @submit.prevent="onBoardEnter">
                <div
                    class="sidemenu-boards-search has-feedback"
                    :class="search ? 'filled' : 'empty'"
                >
                    <input
                        type="text"
                        autocorrect="off"
                    ...
                    <span
                        :class="
                            search ? 'glyphicon-remove' : 'glyphicon-search'
                        "
                        @click="search = ''"
                        class="search-clear glyphicon form-control-feedback"
                    ></span>
                </div>
            </form>
                        ...
                        >
                            ...
                            <span class="pull-right sidemenu-board-icons">
                                <span
                                    ...
                                    ><i class="fa fa-eye"></i
                                ></span>
                                <span v-if="board.isFavourite" title="Избранная"
                                    ><i class="fa fa-heart-o"></i
                                ></span>
                            </span>
                            <span class="sidemenu-board-title">{{
                                board.name
                            }}</span>
                            <span class="sidemenu-board-dir"
                                >&mdash; {{ board.dir }}</span
                            >
                        </content-link>
                    </div>
                </div>
...
export default {
    ...
    created() {
        ...
        this.$bus.on(BusEvents.TOGGLE_SIDEBAR, (isVisible) => {
    ...
    methods: {
        ...
        forceBoardRoute(route) {
            if (
                ...
                route.params.dir === this.$route.params.dir
            ) {
                this.$bus.emit(BusEvents.REFRESH_CONTENT);
                this.visible = false;
            } else {
        ...
        onBoardEnter() {
            ...
            if (!boards || !boards.length) {
                if (this.search.match(/^[a-z0-9]+$/i)) {
                    this.forceBoardRoute({
                        name: "board",
                        params: { dir: this.search },
                    });
                }
                return;
            }
        ...
        filteredBoards() {
            try {
                ...
                return this.boardList.filter((board) => {
                    return (
                        board.dir.match(searchRE) || board.name.match(searchRE)
                    );
                });
            } catch (e) {
