<template>
    <div id="app">
        ...
        <ContentLayer
            id="content"
            :class="{ blurred: blurred, [contentClass]: true }"
        />
    </div>
</template>
    ...
    created() {
        ...
        $(document).click((event) => {
            if (
                ...
                event.target.tagName.toLowerCase() === "address"
            ) {
            ...
            if (
                ...
                event.target.className.toLowerCase() === "boardref"
            ) {
                event.preventDefault();
                const address =
                    event.target.attributes.getNamedItem("data-board").value;
    ...
    mounted() {
        if (!("" + document.cookie).match(/welcome_text/)) {
            setTimeout(() => {
                ...
                document.cookie =
                    "welcome_text=true; expires=" + cookieDate.toUTCString();
            }, 1500);
        }
