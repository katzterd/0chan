                    ...
                    <tr v-for="log in logs">
                        ...
                        <td>
                            <router-link
                                :to="{
                                    name: 'board',
                                    params: { dir: log.board.dir },
                                }"
                                >/{{ log.board.dir }}/</router-link
                            >
                        </td>
                        <td>{{ log.user }}</td>
                        <td>{{ log.event }}</td>
                        <td>
                            <div v-if="log.info">
                                ...
                                <div v-if="log.info.threadId">
                                    <router-link
                                        :to="{
                                            ...
                                            params: {
                                                dir: log.board.dir,
                                                threadId: log.info.threadId,
                                            },
                                        }"
                                    >
                                        Тред №{{ log.info.threadId }}: "{{
                                            log.info.threadTitle
                                        }}"
                                    </router-link>
                                </div>
                                ...
                                <div v-if="log.info.attachmentId">
                                    Пост
                                    <a :data-post="log.info.postId"
                                        >&gt;&gt;{{ log.info.postId }}</a
                                    >
                                    <div v-if="log.info.attachment">
                                        <PostAttachment
                                            v-if="log.info.attachment"
                                            :moderatable="true"
                                            :no-scroll="true"
                                            :attachment="log.info.attachment"
                                        ></PostAttachment>
                                    </div>
                                ...
                                <div v-if="log.info.ban">
                                    ...
                                    Выдан:
                                    {{ log.info.ban.bannedAt | timestamp }}
                                    <br />
                                    До:
                                    <span
                                        :class="{
                                            'text-deleted':
                                                log.info.ban.unbannedAt,
                                        }"
                                    >
                                        {{
                                            log.info.ban.bannedTill | timestamp
                                        }}</span
                                    >
                                ...
                                <div v-if="log.info.change">
                                    <b>{{ log.info.change.name }}</b
                                    >: {{ log.info.change.old }} &rarr;
                                    {{ log.info.change.new }}
                                </div>
                            </div>
                        </td>
...
export default {
    ...
    methods: {
        ...
        onMessageSubmit() {
            ...
            Moderation.message(this.dir, this.message).then((response) => {
                if (response.data.ok) {
                    this.message = "";
                    this.fetch();
                }
