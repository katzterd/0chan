    ...
    <div v-if="bans">
        <div class="">
            <form
                ...
                @submit.prevent="onBanSearchSubmit"
                @reset="onBanSearchReset"
            >
                Поиск по ID:
            ...
            <table class="table">
                ...
                <tbody>
                    <tr v-for="ban in bans">
                        ...
                        <td>
                            <div v-if="ban.isActive" class="label label-danger">
                                Активен
                            </div>
                            <div
                                v-else-if="!ban.isActive"
                        ...
                        <td>
                            <router-link
                                :to="{
                                    name: 'board',
                                    params: { dir: ban.board.dir },
                                }"
                                :title="ban.board.name"
                                >/{{ ban.board.dir }}/</router-link
                            >
                        </td>
                        <td>
                            {{ ban.reason }}
                            ...
                            />
                            <span v-if="!ban.post"
                                ><i>(пост уже удален)</i></span
                            >
                        </td>
                        <td>
                            {{ (ban.bannedTill - ban.bannedAt) | timespan }}
                            <span class="text-muted"
                                >(до {{ ban.bannedTill | timestamp }})</span
                            >
                            <br /><br />
                        ...
                        <td>
                            <div v-if="ban.appeal">
                                <b>Обжалование</b>: <br />{{ ban.appeal }}
                            </div>
                            <div v-if="!ban.appeal">
                                Обжалование не подавалось
                            </div>
                            <div class="vspace2"></div>
                            <div v-if="ban.isActive" style="text-align: right">
                                <button
                                    class="btn btn-default"
                                    @click.prevent="removeBan(ban)"
                                >
...
export default {
    ...
    methods: {
        ...
        removeBan(ban) {
            ...
            return Moderation.removeBan(ban.id).then((response) => {
                if (response.data.ok) {
