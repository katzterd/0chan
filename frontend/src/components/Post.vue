        ...
        >
            ...
            >
                ...
                <span class="post-id">
                    ...
                    <router-link
                        :to="threadRoute"
                        @click.native="replyToPost(post.id)"
                    >
                        #{{ post.id }}
                    </router-link>
                ...
                <span class="pull-right">
                    <span
                        ...
                        class="post-thread-options"
                    >
                        <span
                            v-if="thread.isBumpLimitReached"
                            title="Достигнут бамп-лимит"
                        >
                            <i
                                class="fa fa-fw fa-arrow-up active"
                                style="text-decoration: overline"
                            ></i>
                        </span>
                        <span
                            v-if="thread.purgedAt"
                            @click="post.canBeModerated && unpurgeThread()"
                        >
                            <i
                                class="fa fw-fw fa-trash"
                                v-if="thread.purgedAt !== -1"
                                :title="purgeDateTitle"
                                :class="{
                                    active: true,
                                    hand: post.canBeModerated,
                                }"
                            ></i>
                            <i
                        ...
                        <span @click="post.canBeModerated && togglePin()">
                            <i
                                ...
                                "
                                :class="{
                                    active: thread.isPinned === true,
                                    hand: post.canBeModerated,
                                }"
                            ></i>
                            <i
                                class="fa fa-spinner fa-spin fa-fw"
                                v-if="thread.isPinned === null"
                            ></i>
                        </span>
                        <span @click="post.canBeModerated && toggleLock()">
                            <i
                                class="fa fw-fw fa-lock"
                                :title="thread.isLocked ? 'Закрыть' : 'Закрыть'"
                                v-if="
                                    thread.isLocked === true ||
                                    (post.canBeModerated &&
                                        thread.isLocked !== null)
                                "
                                :class="{
                                    active: thread.isLocked === true,
                                    hand: post.canBeModerated,
                                }"
                            ></i>
                            <i
                                class="fa fa-spinner fa-spin fa-fw"
                                v-if="thread.isLocked === null"
                            ></i>
                        </span>
                    </span>
                    <span
            ...
            <div
                class="post-body"
                :class="{ 'post-inline-attachment': isInlineAttachment }"
                v-if="!isHidden || readonly"
            >
                <div
                    class="post-attachments"
                    v-if="post.attachments.length && !isHidden"
                >
                    <PostAttachment
                ...
                <div class="post-body-message">
                    <div
                        ...
                        v-if="
                            post.parentId &&
                            post.parentId != post.opPostId &&
                            !tree
                        "
                    >
                        <a :data-post="post.parentId"
                            >&gt;&gt;{{ post.parentId }}</a
                        >
                    </div>
                    <div v-html="post.messageHtml" v-once></div>
                </div>
                <div v-if="post.isUserBanned" class="user-was-banned">
                    (USER WAS BANNED FOR THIS POST)
                </div>
                <div
                    v-if="!tree && !post.isOpPost && post.repliedByIds.length"
                    class="post-replied-by"
                >
                    Ответы:
                    <span v-for="replyPostId in post.repliedByIds">
                        <a :data-post="replyPostId">&gt;&gt;{{ replyPostId }}</a
                        ><span>, </span>
                    </span>
                </div>
                <div
                    v-if="post.referencedByIds.length"
                    class="post-referenced-by"
                >
                    Упоминания:
                    <span v-for="refPostId in post.referencedByIds">
                        <a :data-post="refPostId">&gt;&gt;{{ refPostId }}</a
                        ><span>, </span>
                    </span>
                </div>
            </div>
            ...
            <div class="post-footer" v-if="!readonly">
                ...
                <div
                    class="pull-left"
                    v-if="!post.canBeModerated && post.canBeReported"
                >
                    <span
                        class="post-button"
                        @click="reportPost"
                ...
                >
                    ...
                    ></span>
                    <span class="post-button" v-if="post.isDeleted === null"
                    ...
                    <div
                        v-if="isBanFormShown"
                        class="ban-popup panel panel-default block"
                        slot="content"
                    >
                ...
                <div class="pull-right" v-if="!post.isDeleted">
                    <span v-if="post.user" class="text-muted post-date">{{
                        post.user.name
                    }}</span>
                    <span
                        class="post-button"
                        v-if="thread.board.likes == true"
                        @click="ratePost(true)"
                        title="Нульчую"
                    ...
                    >
                    <span
                        class="post-button post-dislike"
                        v-if="thread.board.likes == true"
                        @click="ratePost(false)"
                        title="НСБ"
                        ><i class="fa fa-angle-down"></i>
                        {{ post.likes.disliked }}</span
                    >
                    <span
                        class="post-button post-button-reply"
                        @click="toggleReplyForm"
                        v-if="post.canReplyTo"
                    ...
                    >
                        <i
                            v-if="!isFetchingBanInfo"
                            class="fa fa-fw fa-gavel"
                        ></i>
                        <i
                            v-else-if="isFetchingBanInfo"
...
export default {
    ...
    methods: {
        ...
        deleteAllPosts() {
            ...
            Moderation.deleteAllPosts(this.post.id).then((response) => {
                if (response.data.ok) {
                    this.$bus.emit(BusEvents.REFRESH_POSTS, response.data.posts);
                }
            });
        },
        ratePost(isLike = false) {
            Post.rate(this.post.id, isLike).then(
                (response) => {
                    if (response.data.ok) {
                        if (isLike) {
                        ...
                        switch (response.data.error) {
                            ...
                            case "likes_disabled":
                                this.$bus.emit(
                                    ...
                                    "Рейтинг на этой доске отключен"
                                );
                            ...
                            case "op_match":
                                this.$bus.emit(
                                    BusEvents.ALERT_ERROR,
                                    "Нельзя ставить оценки на свой пост"
                                );
                                break;
                        }
                    }
        ...
        reportPost() {
            ...
            if (!reason) return;
            Moderation.reportPost(this.post.id, reason).then((response) => {
                if (response.data.ok) {
                    this.$bus.emit(
                        BusEvents.ALERT_SUCCESS,
                        "Спасибо, мочераторы оповещены"
                    );
                } else {
                    this.$bus.emit(
                        BusEvents.ALERT_ERROR,
                        "Ошибка - причина не заполнена (или слишком длинная)"
                    );
                }
            });
        },
        ...
        togglePin() {
            ...
            Moderation.pinThread(this.thread.id, !isPinned).then((response) => {
                if (response.data.ok) {
        ...
        showBanInfo() {
            ...
            Moderation.banInfo(this.post.ban).then((response) => {
        ...
        unpurgeThread() {
            ...
            return Moderation.unpurgeThread(this.thread.id).then((response) => {
                if (response.data.ok) {
    ...
    mounted() {
        ...
        this.$bus.on(BusEvents.REFRESH_POSTS, (posts) => {
            const post = posts.find(p => p.id === this.post.id);
            if (post) {
    ...
    computed: {
        ...
        imRoute() {
            return {
                name: "im_start",
                params: { toAddress: this.post.identity.address },
            };
        },
        purgeDateTitle() {
            let title =
                "Отмечен к удалению в " +
                Filters.timestamp(this.thread.purgedAt);
...
.post {
    ...
    .post-body {
        ...
        .post-replied-by + .post-referenced-by {
            padding-top: 0;
        }
    }
