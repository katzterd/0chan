    ...
    <div v-if="board !== null">
        <Headline>
            ...
            <div slot="buttons" class="btn-group" v-if="board.dir">
                ...
                >
                    <i
                        :hidden="board.isFavourite !== null"
                        class="fa fa-spinner fa-spin fa-fw"
                    ></i>
                    <i
                        :hidden="board.isFavourite !== false"
                        class="fa fa-fw fa-heart-o"
                    ></i>
                    <i
                        :hidden="board.isFavourite !== true"
        ...
        <div
            ...
            v-show="newThreadForm && board.canCreateThread"
        >
            <div>
        ...
        <div
            class="threads-scroll-spy hidden-md hidden-sm hidden-xs"
            v-show="threads.length > 0 && !newThreadForm"
            ref="scrollSpy"
        >
            <div class="threads-scroll-toggler text-right">
                <i
                    class="fa fa-angle-left"
                    v-if="!storage.isThreadListVisible()"
                    @click="storage.setThreadListVisibility(true)"
                ></i>
                <i
                    class="fa fa-angle-right"
                    v-if="storage.isThreadListVisible()"
                    @click="storage.setThreadListVisibility(false)"
                ></i>
            </div>
        ...
        <div v-if="board && !threads.length">
            <div class="empty-page">
                ...
                <div v-if="type == 'watched'">
                    Чтобы добавить сюда тред, нужно нажать в нем кнопку
                    <i class="fa fa-star"></i>.
                </div>
                <div v-if="type == 'favourite'">
                    Чтобы подписаться на доску, нужно нажать в ней кнопку
        ...
        <div class="vspace2 text-center" v-if="pagination.hasMore">
            <VisibilityTracker @enter="getMoreThreads" :area="200">
                <button
                    class="btn btn-default"
                    @click.prevent="getMoreThreads"
                    :disabled="fetchingMore"
                >
                    <span v-if="fetchingMore">
                        <i class="fa fa-spinner fa-spin fa-fw"></i> подгружаем
                        еще треды
                    </span>
                    <span v-if="!fetchingMore">
...
export default {
    ...
    methods: {
        ...
        toggleFavourite() {
            ...
            Board.favourite(this.board.dir, !isFav).then((response) => {
        ...
        showBoardInfo() {
            this.$bus.emit(BusEvents.SHOW_MODAL, BoardInfo, {
                board: this.board,
            });
        },
        updateThreadScrollSpy() {
            ...
            const scrollPos =
                ...
                (window.document.body.scrollHeight - viewH);
            ...
            this.$refs.scrollSpy.style.top =
                offset - overflow * scrollPos + "px";
        },
    },
};
</script>
...
.threads-scroll-spy {
    ...
    &:hover {
        ...
        li:hover,
        li.active {
            background: rgba(desaturate($input-border-focus, 50%), 0.1);
        }
    }
