...
export default {
    ...
    addModerator(boardDir, login) {
        return Api.get("management/addModerator", {
            params: { board: boardDir, user: login },
        });
    },
    removeModerator(boardDir, login) {
        return Api.get("management/removeModerator", {
            params: { board: boardDir, user: login },
        });
    },
    board(boardDir, form) {
        ...
        if (form) {
            return refreshSidebarAfter(
                Api.post("management/board", form, params)
            );
        } else {
    ...
    deleteBoard(boardDir) {
        return refreshSidebarAfter(
            Api.get("management/deleteBoard", { params: { board: boardDir } })
        );
    },
    changeOwner(boardDir, newOwnerLogin) {
        return refreshSidebarAfter(
            Api.get("management/changeOwner", {
                params: { board: boardDir, newOwner: newOwnerLogin },
            })
        );
    },
};
